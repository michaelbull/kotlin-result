 ...
 *
 * This is the fallible equivalent of [Flow.runningFold][kotlinx.coroutines.flow.runningFold] and
 * the [Flow] equivalent of
 * [Iterable.tryRunningFold][com.github.michaelbull.result.tryRunningFold].
 */
public suspend inline fun <T, R, E> Flow<T>.tryRunningFold(
    initial: R,
    crossinline operation: suspend (acc: R, T) -> Result<R, E>,
): Result<List<R>, E> {
    val list = ArrayList<R>().apply { add(initial) }
 ...
 * starts with [initial] value, returning early with the first [Err] if an [operation] fails.
 *
 * This is the fallible equivalent of [Flow.scan][kotlinx.coroutines.flow.scan] and the [Flow]
 * equivalent of [Iterable.tryScan][com.github.michaelbull.result.tryScan].
 *
 * This function is an alias of [tryRunningFold].
 */
public suspend inline fun <T, R, E> Flow<T>.tryScan(
    initial: R,
    crossinline operation: suspend (acc: R, T) -> Result<R, E>,
): Result<List<R>, E> {
 ...
 * Returns a [Result] containing a list of successive accumulation values generated by applying
 * the fallible [operation] from left to right to each element and current accumulator value that
 * starts with the first element of this flow, returning early with the first [Err] if an
 * [operation] fails.
 *
 * This is the fallible equivalent of
 * [Flow.runningReduce][kotlinx.coroutines.flow.runningReduce] and the [Flow] equivalent of
 * [Iterable.tryRunningReduce][com.github.michaelbull.result.tryRunningReduce].
 */
public suspend inline fun <S, T : S, E> Flow<T>.tryRunningReduce(
